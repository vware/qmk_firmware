...
uint8_t               led_state[LED_MATRIX_LED_COUNT];  // LED state buffer
...
void update_pwm_channels(PWMDriver *pwmp) {
    for (uint8_t col_idx = 0; col_idx < LED_MATRIX_COLS; col_idx++) {
        ...
        // TODOOOO Scan the key matrix column ######################################################################################################
...
void led_callback(PWMDriver *pwmp) {
    ...
    // TODOOOOOOOOOO Scan the key matrix row ###########################################################################################
